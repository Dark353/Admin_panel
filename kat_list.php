...
<?php checkSession();?>
<?php
YetkiKontrol::kullkont($_SESSION['is_admin'], YetkiKontrol::CREAD);
...
<body class="dark-mode-variables">
        ...
        <main>
            <div class="recent-orders">
                ...
                <button onclick="document.getElementById('id01').style.display='block'" class="w3-button  w3-large"
                    style="border-radius: 18px; background-color: #202528;">Kategori Ekle</button>
                <?php }?>
                <hr>
                ...
                <table>
                    ...
                    <tbody id="surukle">
                        <?php
                           $sorgu = $conn -> prepare("select * from tbl_kat order by yer");
            ...
            while($veri = $respons->fetch_assoc()){
                ?>
                        ...
                        <tr id="<?=$veri['id']?>">
                            ...
                            <td>
                                <?php
                                    if (isset($dizi_category[$veri["ust"]])) {
                                        echo $dizi_category[$veri["ust"]] . " (" . $veri['ust'] . ")";
                                    } else {
                                        echo "Üst veri yok";
                                    }
                                    ?>
                     ...
                     if ($_SESSION['is_admin'] & YetkiKontrol::CUPDATE) {?>
                                <?php if($veri["durum"]) {?>
                                <a href="./inc/kat_con.php?islem=pasifakfif&sira=<?=$veri['id']?>&durum=0"
                                    class="bi bi-eye" style="color: green; margin-right: 7px;"></a>

                                <?php } else {?>
                                <a href="./inc/kat_con.php?islem=pasifakfif&sira=<?=$veri['id']?>&durum=1"
                        ...
                        if ($_SESSION['is_admin'] & YetkiKontrol::CDELETE) {?>
                                <a href="./inc/kat_con.php?islem=delete&id=<?=$veri['id']?>"><i class="bi bi-trash3"
                                        style="color: red; "></i></a>
                                <?php }?>
                            </td>
                        </tr>
                        <script>
                        <?php if ($_SESSION['is_admin'] & YetkiKontrol::CUPDATE) {?>
                        $(function() {
                            $("#surukle").sortable({
            ...
            <form action="./inc/kat_con.php" method="post" id="yeni_yerler">
                <input type="hidden" name="f_siralar" id="f_siralar">
                <input type="hidden" name="islem" value="katsirala">
            </form>
    ...
    <?php include './include/kat_ekleM.php'?>
